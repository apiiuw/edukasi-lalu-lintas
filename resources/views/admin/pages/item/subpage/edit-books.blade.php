...
<div class="p-4 sm:ml-64 bg-gray-200 min-h-screen flex justify-center items-center">
    <div class="p-6 rounded-lg w-full max-w-6xl mt-5 lg:mt-24">
        <h1 class="text-center text-xl font-semibold mb-4">EDIT ELEKTRONIK BUKU</h1>

        <form id="uploadForm" action="{{ route('admin.update.book', ['id' => $book->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')

            <label class="block font-medium">Judul Buku</label>
            <input value="{{ $book->judul }}" type="text" name="judul" class="w-full border border-gray-400 rounded-xl p-2 mb-4 placeholder:text-gray-500" placeholder="Ketikkan Judul..." required>

            <label class="block font-medium">Tahun Rilis</label>
            <input value="{{ $book->tahun_rilis }}" type="number" name="tahun_rilis" class="w-full border border-gray-400 rounded-xl p-2 mb-4 placeholder:text-gray-500" placeholder="Ketikkan Tahun Rilis..." required>

            <label class="block font-medium">Deskripsi Buku</label>
            <textarea name="deskripsi" class="w-full border border-gray-400 rounded-xl p-2 mb-4 placeholder:text-gray-500" rows="4" placeholder="Ketikkan Deskripsi Buku..." required>{{ $book->deskripsi }}</textarea>
            ...
            <input type="hidden" name="kata_kunci" id="kataKunciField" value="{{ $book->kata_kunci }}">            
            ...
            <div class="flex items-center border border-gray-400 rounded p-2 bg-white gap-2">
                ...
                <input value="{{ $book->cover }}" type="file" name="cover" id="coverInput" accept="image/png, image/jpeg, image/jpg" class="hidden" onchange="previewImage(this)" required>
            </div>
            ...
            <div class="flex items-center border border-gray-400 rounded p-2 bg-white gap-2">
                ...
                <input value="{{ $book->pdf }}" type="file" name="pdf" id="pdfInput" accept="application/pdf" class="hidden" onchange="showPDFName(this)" required>
            </div>

            <button type="submit" class="w-full bg-blueJR text-white py-2 rounded-xl mt-4">Simpan Perubahan</button>
        </form>
